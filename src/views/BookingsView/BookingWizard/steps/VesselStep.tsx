...
function matchesVessel(vessel: Vessel, query: string): boolean {
...
export default function VesselStep({ vessels, selectedId, onSelect, loading }: VesselStepProps) {
  ...
  const filteredVessels = useMemo(
    () => vessels.filter((vessel) => matchesVessel(vessel, search)),
    [vessels, search],
  );
  ...
  return (
    <div className="space-y-4">
      <WizardStepSearch
        value={search}
        onChange={setSearch}
        placeholder="Buscar barco, clase…"
      />
      {filteredVessels.length === 0 ? (
        <p className="rounded-xl border border-dashed border-zinc-200 px-4 py-8 text-center text-sm text-zinc-500 dark:border-zinc-700">
          No hay barcos que coincidan con la búsqueda.
        </p>
      ) : (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
